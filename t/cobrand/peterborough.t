...
subtest 'open311 bartec request handling with asset', sub {
    FixMyStreet::override_config {
        STAGING_FLAGS => { send_reports => 1 },
        ALLOWED_COBRANDS => ['peterborough' ],
        MAPIT_URL => 'http://mapit.uk/',
        ...
        $p->push_extra_fields({ name => 'central_asset_id', value => '1234'});
        $p->push_extra_fields({ name => 'asset_details', value => 'an asset'});
        ...
        my $req = $test_data->{test_req_used};
        my $c = CGI::Simple->new($req->content);
        is $c->param('attribute[description]'), "Title Test 1 for " . $peterborough->id . " Detail\r\n\r\nAsset id: 1234\r\nAsset detail: an asset", 'asset details added to description';
...
subtest 'open311 bartec request handling with no asset', sub {
    FixMyStreet::override_config {
        STAGING_FLAGS => { send_reports => 1 },
        ALLOWED_COBRANDS => ['peterborough' ],
        MAPIT_URL => 'http://mapit.uk/',
        ...
        my $req = $test_data->{test_req_used};
        my $c = CGI::Simple->new($req->content);
        is $c->param('attribute[description]'), "Title Test 1 for " . $peterborough->id . " Detail", 'no asset details added to description';
        is $c->param('attribute[postcode]'), 'ZZ1 1ZZ', 'postcode sent';
        is $c->param('attribute[house_no]'), '23', 'house number sent';
        is $c->param('attribute[street]'), 'A Street', 'street sent';
    };
